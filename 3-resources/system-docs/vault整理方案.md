...
> 归宿：审批并执行完成后 → `3-resources/system-docs/`（作为 vault 结构的决策记录长期保留；第一阶段迁移表中「及本方案文件」一行以此为准）。
...
### 交付物「归宿」头约定（今后所有落进 inbox 的文件遵守）

凡是 AI 交付到 inbox 的文件（调研报告、草稿、方案、周报以外的一切），第一行正文必须是一行处置头：

```
> 归宿：<处理完后该去哪>
```

例如 `> 归宿：读完删`、`> 归宿：1-projects/AI谋生/`、`> 归宿：3-resources/notes/AI/`。这样每周清 inbox 时不需要重新思考每个文件是什么——打开看第一行，照做或否决，30 秒一条。没有归宿头的文件视为约定违例，清理时优先处理（补一行或直接归档）。本方案文件自身的归宿头见文首。
