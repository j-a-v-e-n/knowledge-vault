...
**最后更新**：2026-05-18 | **页面总数**：90 | **最新操作**：编译 3 篇 ECE284 数字公共卫生监测主题文献 — [[Anglemyer_2020_数字接触追踪Cochrane综述]] (Cochrane SR 12 篇研究 digital contact tracing 实证 largely unproven) + [[Radin_2020_Fitbit流感监测]] (Fitbit 47K 用户 + 13.3M RHR 测量 5 州 ILI nowcasting r=0.84-0.97) + [[Minakshi_2020_智能手机蚊媒识别]] (smartphone + IRV2 transfer learning 蚊媒 species 80% accuracy, 最致命 vector Ae.aegypti+An.stephensi 准确率最高)；前序 5/15 编译 2 篇 COGS117 Week 7 文献 — [[Shafto_2012_社会学习贝叶斯框架]] (Bayesian formal model of social learning，3 个 context 区分) + [[Herrmann_2007_文化智力假说]] (PCTB 跨物种比较实证支持 Cultural Intelligence Hypothesis)；前序 5/14 BS/MS SoP final + degree-progress.md 编译，5/11 抖音 8 视频批量编译（Javen 喂入 8 个 Untitled.md 字幕）— Claude Code 8 项新功能 / Claude Code permissions / DeepSeek TUI 登顶 GitHub / Antirez 本地推理 / AI 落地咨询师岗位 / CodeBanana / SEQUENCE OS / Codex 三件套 PPT。spawn 3 个 researcher 并行调研验证事实（Code with Claude 2026 大会真实存在 5/6 SF；DeepSeek-TUI 真 24.7k stars 由 Hunter Bown 做；Antirez ds4 GitHub 真有 5/7 发布；Anthropic FDE 真招聘 $180-550K；CodeBanana 真出门问问 2024 发布）。新增 8 source + 3 concept + 2 synthesis + 1 overview 总共 **14 个新页面**。前序操作：5/12 ECE284 Med-HALT 编译 + 演讲稿
...
- [[Anglemyer_2020_数字接触追踪Cochrane综述]] — Anglemyer et al. (2020 Cochrane SR)：数字 contact tracing 在 epidemics 中有效性 rapid review；12 篇研究（6 cohort + 6 modelling）**无 1 RCT**；modelling 给 low-certainty 证据 R_eff 减 18-26%（manual 35-53% 更强）；cohort 显示 digital 找 contacts 2-3.5x 多但 certainty very low；South Korea/Singapore 成功不能归因 app 本身；equity + privacy 红线；结论 "effectiveness largely unproven"
- [[Radin_2020_Fitbit流感监测]] — Radin/Topol team (2020 Lancet Digital Health)：47,249 Fitbit 用户 + 13.3M RHR/sleep 测量 → 5 states 流感 nowcasting；Fitbit signal 让 CDC ILI prediction Pearson r 平均 +0.12（6.3-32.9%），final r 0.84-0.97；首次证实 wearable population-level surveillance 补足 CDC 1-3 周 lag；但 Fitbit signal **跟随**而非**领先** ILI
- [[Minakshi_2020_智能手机蚊媒识别]] — Minakshi et al. (2020 ACM COMPASS)：25,867 张智能手机图（250 specimens）+ Inception-ResNet V2 transfer learning；genus 级 92-93.5% accuracy；最高 species 准确率出现在 **Ae. aegypti**（Zika/dengue）+ **An. stephensi**（malaria）这两个最致命 vector；feature map heatmap 验证模型看 thorax/wings/abdomen/legs 正确解剖；公共卫生 taxonomy "dying field" 自动化范式

---

## 🤖 AI 跨课程文献

存放跨课程通用 LLM / 评测 / 基础模型相关论文，**不绑定单一课程**。原始文件在 `raw/AI/`，source 页镜像至 `notes/AI/`。

### 文献笔记
- [[Pal_2023_MedHALT]] — Pal, Umapathi & Sankarasubbu (2023 EMNLP/CoNLL, Saama AI Research)：Med-HALT——第一个专门测 LLM 医疗领域**幻觉抗性**的 benchmark；RHT (FCT/NOTA/FQT) + MHT (4 个 PubMed 检索 task) 共 7 个对抗性任务；揭示 **Instruction-tuning Paradox**（LLaMA-2 70B Base 72.33% → Chat 11.26%）+ 闭源 ≠ 更强 + FCT 全军覆没；Scoring +1/−0.25/0 奖励诚实

### 核心概念
- [[LLM 医疗评测]] — 综合两条范式：**能力评测**（MedQA/USMLE-style accuracy；Med-PaLM 2 代表）vs **诚实度评测**（对抗性 hallucination test；Med-HALT 代表）；同一模型在两类 benchmark 上的成绩还没系统报告（research gap）
...
└── 医疗技术/                                      ← new 2026-05-02
    ...
    └── LLM 医疗评测.md                             ← new 2026-05-11 (能力评测 vs 诚实度评测两条范式，Med-HALT + Med-PaLM 2 支撑)

notes/AI/                       ← new 2026-05-11 (跨课程 LLM 文献)
└── Pal_2023_MedHALT.md         ← Med-HALT benchmark 论文 source 页

raw/AI/Med-HALT_Pal_2023/      ← new 2026-05-11
├── Pal_2023_MedHALT.pdf
├── 2023-07_Pal_MedHALT_arxiv_abstract_metadata.md
├── 2023-07_Pal_MedHALT_arxiv_methods_framework.md
└── 2023-07_Pal_MedHALT_arxiv_results_findings.md
...
└── AI/        ← new 2026-05-11 (5 figs: Pal_2023_MedHALT 框架 + Lyme 例子 + Dataset stats + Temperature + RHT/MHT 主结果)
```
...
| **Instruction-tuning Paradox** | [[Pal_2023_MedHALT]] |
| **RHT (Reasoning Hallucination Test)** | [[Pal_2023_MedHALT]] |
| **MHT (Memory Hallucination Test)** | [[Pal_2023_MedHALT]] |
| **Pointwise Score / abstention reward** | [[Pal_2023_MedHALT]] |
| **MedQA / USMLE benchmark** | [[Pal_2023_MedHALT]], [[Singhal_2025_MedPaLM2_演讲]] |
