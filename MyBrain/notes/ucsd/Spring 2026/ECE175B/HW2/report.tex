...
The total network has $\mathbf{NPARAMS}$\,M parameters.

\paragraph{Data and training.} We pool all $\mathbf{NIMAGES}$ chest X-ray images
...
(wall time $\mathbf{WALLTIME}$\,min). Code: see \texttt{train\_vae\_chest\_xray.py}.

\paragraph{Evaluation.} We follow the
PyTorch-Ignite recipe~\cite{ignite_gan_eval} for FID/IS: 1{,}000 samples are drawn from the
prior $\mathcal{N}(0,I)$, decoded, replicated to $3$ channels, and bilinearly upsampled
to $299{\times}299$ for InceptionV3 features. The reference set is $1{,}000$ randomly
selected real chest X-rays under the same pre-processing.
...
Training images & $\mathbf{NIMAGES}$ \\
Parameters & $\mathbf{NPARAMS}$\,M \\
\bottomrule
\end{tabular}
\end{table}
...
Metric & Value \\
\midrule
Inception Score (IS) & $\mathbf{ISVAL}$ \\
Fr\'echet Inception Distance (FID) & $\mathbf{FIDVAL}$ \\
\bottomrule
\end{tabular}
\end{table}
...
Gaussian decoder assumption induces blurry reconstructions, and (ii) a single global
$z\in\mathbb{R}^{128}$ struggles to represent the high-frequency texture of X-ray
projections. A natural extension is to replace the decoder with a hierarchical or
