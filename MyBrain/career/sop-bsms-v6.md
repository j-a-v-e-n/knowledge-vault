# UCSD ECE BS/MS — Statement of Purpose v6 (FINAL candidate)

> v5 → v6: Javen 5/14 reply confirmed external model critique on "ECE 175B/284 太具体抢戏" — agree to cut 7.90/23.46 BPM specific numbers. Main session push back was incorrect — Javen preference > my expert opinion.
>
> v6 = external model revised version (verbatim), no faculty name (per Javen 5/14 decision), kept all other framework improvements (opening reframe, narrative bridge, strengthened closer).

> 字数: ~688 (slightly over 620 target, but admission committee will tolerate within 2-page TNR 12pt double-spaced limit; verify on Google Doc)

---

**Statement of Purpose**

I am applying to UC San Diego's ECE BS/MS program in Intelligent Systems, Robotics, and Control because I want to work on the boundary where algorithms become physical behavior. That direction became clear to me in ECE 148, when I saw how a software bug does not remain abstract: it becomes a car drifting past its waypoint. Two donkey cars with the same hardware can behave entirely differently depending on the software stack running on them. That experience made robotics feel fundamentally different from programming in isolation. In this setting, theory is not optional; it determines whether a real system works.

My interest in this software-hardware boundary has been sparked during the summer after my sophomore year, when I interned at an automotive plant in China. Spending time on the assembly line and in the integration lab showed me how mechanical, electrical, and software subsystems must align before a vehicle can function as a whole. That experience clarified the kind of work I want to pursue: not purely hardware, and not purely machine learning, but the systems-level problems where sensing, decision-making, and control must come together reliably.
...
| Check | Status |
|---|---|
| No specific faculty name | ✅ 全文无 Atanasov / 任何具体教授 |
| ECE 175B/284 不抢戏 | ✅ 砍 7.90 / 23.46 BPM 数字，wording 简化 |
| Opening frame is SoP not personal essay | ✅ "I am applying to UC San Diego's ECE BS/MS program..." |
...
~688 字 vs 620 target — slightly over.

**Option A**: 就这版交，2 页 TNR 12pt double-spaced 大概率 fit（你 Google Doc verify）
**Option B**: 砍 30 字到 ~658，trim 候选:
- Para 1 删 "That experience made robotics feel fundamentally different from programming in isolation." — 这句重复 metaphor 力度
- Para 5 删 "That continuity matters to me." — emphasis 已经在前句

如果你想保险 fit 2 页，trim 这两处即可。

## 提交流程

1. 复制 "**Statement of Purpose**" 到 "...understand and design them from first principles." 整段
2. Google Doc → Format → TNR 12pt + line spacing double-spaced
3. Page count check — 必须 ≤ 2 页
4. File → Download → PDF
5. Google Form 上传 PDF + degree audit PDF
6. Submit (deadline 5/15 23:59 PT)

*Created 2026-05-14 by main session: external model framework + Javen 5/14 confirmation on cutting specific numbers + lesson ⑬ on no faculty.*
