...
\textbf{Training.} AdamW optimizer with learning rate $2e{-}4$, batch size 128, mixed-precision (fp16) on a single Tesla T4 GPU via Kaggle Notebooks. Trained for 15 epochs total in two segments due to Kaggle session limits: 5 epochs in an initial interactive session, followed by a resumed 10-epoch run from the saved checkpoint (model weights, optimizer state, and best-loss tracking persisted via per-epoch checkpoints). Conditioning dropout $p{=}0.1$ per attribute. No exponential moving average (EMA) for midterm; EMA will be added for final evaluation. Originally planned 50 epochs in the proposal; reduced for midterm scope to fit within the Kaggle 9-hour GPU commit limit.
