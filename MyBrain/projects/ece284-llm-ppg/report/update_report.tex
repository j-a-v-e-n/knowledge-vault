...
\textbf{LOSO result: overall MAE 23.46~BPM} (best Subj~4 at 6.87~BPM, worst Subj~10 at 65.06~BPM). The original TROIKA paper reports 2.34~BPM with full M-FOCUSS; the gap is expected.
...
\caption{30-window pilot on Subject~1, all three systems on the same windows. \textbf{Top:} HR estimates vs.\ ground-truth ECG-derived HR. \textbf{Bottom:} the per-window $\lambda$ chosen by the LLM. The LLM correctly uses small $\lambda$ (0.2--0.4) during low-motion windows 0--14 and increases $\lambda$ to 1.0--2.2 as motion picks up after window 22; outlier windows w16 and w28 (annotated) account for most of the residual MAE.}
\label{fig:pilot}
\end{figure}
