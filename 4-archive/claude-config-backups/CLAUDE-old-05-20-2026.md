...
- **Override** `wiki/工程方法/AI 团队设计原则.md` 中 "default single-agent" axiom：那条仅适用于**纯执行任务**；本规则 escalate 重大 decision 必须 multi-agent。Default 改为：纯执行 single-agent / 决策 multi-agent
- **加强** "Deliverable edit 完强制 Check 步骤"：那条是 **post-edit** PDCA check；本条是 **pre-decision** second opinion
- **强化** "三大 self-improvement axiom 中记教训"：本条规则本身就是 lesson learned from LOSO 单 agent launch
