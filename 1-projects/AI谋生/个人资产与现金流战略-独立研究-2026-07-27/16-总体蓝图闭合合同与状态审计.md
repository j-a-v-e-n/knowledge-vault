...
状态：`R3_TWO_LEG_FAIL / R4_REMEDIATION / DESIGN_ONLY / NO_EXTERNAL_ACTION_AUTHORITY`
...
- 本 R4 对机会线只绑定已发布的 control-plane snapshot `0180f144c2fbdd12cee9bf3c3095331dc57cb7651a336643ee80adaa1e6dd502` 与 `STATE.json` `57a09e085b464e639247429b583d1d1579342a3401fe84ee932b446ead92a317`；发布窗口结束后 live write freeze 已解除。后续 live root 变化只形成尚未审查的新版本，reviewer 不持续监控 live root，也不得把它回写为本 snapshot 的全局失败。
- 依赖方向只能是 portfolio 指向已发布 workflow snapshot。机会线 live root 与 portfolio exact candidate 不得互相绑定；任何新 snapshot 只在短暂发布握手中冻结、给出 closed inventory/hash，随后 reviewer 只读该发布物。
...
| AI 项目 Harness | scope 拓扑与最小字段已定义；R3 证明权威、blocker 和动作类型缺口已关闭，但 recovery oracle、stale fixture、时间顺序和 blind protocol 仍失败，R4 必须用 append-only blind checkpoint → candidate-bound conformance receipt → prior-receipt-bound adversarial receipt → verified successor 的顺序精确修复 | `CONTESTED` |
...
| 完成审计 | R1、R2、R3 总体均 FAIL；R3 两份拒绝 receipt 已持久化。尚缺 R4 blind candidate、distinct conformance acceptance 与 exact adversarial challenge，owner constraints 仅阻塞之后的实际排序 | `OPEN_BLOCKING` |

## R3 FAIL 后的 R4 关闭条件

- R3 的 adversarial 与 blind-recovery legs 均已 FAIL；不得用新 reviewer 覆盖、删除或重解释这两个结果。R4 必须绑定 R3 request 与两份 verbatim receipt 的 exact hashes；
...
- `TASK_PROJECT_RECOVERY_READ_ONLY` 必须把条件式 tie-break、review successor rule 和 evidence timeline 纳入机械 projection；wrong tie-break 与 wrong successor 的定向 fixture 必须失败；
- stale fixture 必须只产生一个目标 blueprint 的实际 hash mismatch，不能因相对路径错误把 `MISSING` 假阳性当作 stale pass；state `as_of` 必须不早于所有绑定 observation，且 supplemental C8 有独立 observed timestamp；
- R4 blind generator 只能读取 blind request、适用根指令、task-class manifest 与其冻结 input bundle；它既不能打开也不能 hash gold/rubric/schema/tests/fixtures 或历史 review request/receipt，只能返回 `CANDIDATE_RECOVERY` 或 `REFUSAL`，不能签发 PASS；
- blind generator 返回后，primary agent 必须在任何 gold/conformance feedback 前把 exact JSON bytes 作为 append-only checkpoint 持久化并重算 SHA-256/byte length；随后 distinct conformance reviewer 才可读取 answer-bearing artifacts、运行完整 suite，并在 receipt 中绑定 blind hash、extracted-object hash、input bundle、state 与 request；conformance PASS 的 exact receipt 持久化后，另一 adversarial reviewer 才可绑定前两份 hash 并独立复查 exact blueprint/state/request、R3 findings、双控制面、循环 Gate、审查输出持久化、过度治理和可执行性；
- 三个角色必须是无旧聊天/memory 的 fresh identities。任何 blind refusal/contamination、conformance 或 adversarial 非 PASS、Critical/Major、hash drift 或 fixture failure 都保持总体 `FAIL`；blind output 本身不是签发 receipt；
- review admission 只要求 exact inputs 与 admission blockers 为零。各 stage 通过 orchestrator 返回输出；primary agent 每收到一个 stage 就必须逐字、append-only 持久化，不能改写发现或等全部 stage 完成后再补历史。Blind refusal/taint 与 conformance nonpass 均可短路为 FAIL；只有 conformance PASS 才运行 adversarial stage；
- `review-protocol/verify_r4_review_attempt.py` 必须重算 artifact hash/byte length、核对 receipt identity 与 findings、机械派生 outcome。先物化并验证独立 outcome candidate 与 exact temporary successor，再把完全相同的 successor bytes 提交到 `17`；successor 只能改动封闭枚举的 review transition 字段，其余 predecessor 值逐值保留，两个分支均不得扩权或激活 workflow；不能用路径存在、文件名中的 “acceptance” 或自然语言 PASS 代替绑定；
- R4 通过后，下一 primary action 才是向 Javen 取得会改变 portfolio 排序的最小 owner constraint；这不自动激活任何 workflow。对外身份、精确对象、价格/合同等只在对应 activation 临近时询问；
