...
- 状态：`EXHAUSTIVE-LEDGER-VERIFIED / FINAL-STATUS-INDEPENDENT-ACCEPTANCE-PENDING`
...
```bash
python3 -B verify_run2_crosswalk.py
python3 -B test_run2_crosswalk.py -v
```

验证器要求 S1 最终 CE-IN `131` 条、S2 最终 CE-IN `141` 条，逐条 identity 唯一；缺失、重复、claim/scope/K 篡改、把 unused row 自行晋级为 load-bearing 或非 canonical JSONL 均 fail closed。
...
| `S1/K07/R01/turn153search0` | Lean Startup 被描述为假设驱动实验法，但其直接促进组织学习的证据有限 | 限于可见综述；采用实验法不等于需求、付款或盈利；`INCLUDE/UNSUBTYPED` | `RQ9`；限制 `EF-16`，只支持有纪律测试的窄边界 | `DD-06,DD-10` | 各行业中的学习与业务结果效应未知 |
| `S1/K07/R03/turn153search2` | Lean Startup 的适用行业、组织适配与采用驱动呈现情境异质性 | 不证明普遍绩效；`INCLUDE/UNSUBTYPED` | `RQ9`；只限制 `EF-16` 的跨行业/组织普遍化 | `DD-06` | 适用场景、选择偏差与业务结果未知 |
...
| `S1/K09/R16/turn155search11` | EdgeBench 项目页描述以真实科研数据和跨模块任务评测超长时程环境学习 | 只证明项目页所述 benchmark 工件；不证明一般 Agent 可靠；`INCLUDE/UNSUBTYPED` | `RQ6,RQ7`；只支持 `EF-10` 的长时程/终态评测窄机制并限制外推 | `DD-08,DD-10,DD-13` | benchmark validity、结果、oracle 与生产可靠性未知 |
...
| `S1/K12/R19/turn159search18` | 大学/监管提交材料把 AI 驱动 dark patterns 与隐私、民主和经济伤害联系起来 | 支持风险论证与治理诉求；不是总体实证估计；`INCLUDE/UNSUBTYPED` | `RQ5`；只支持 `SS-05` 的操纵/伤害机制边界，不支持发生率 | `DD-12` | 发生率、因果强度、受影响群体与缓解效果未知 |
...
| `S2/K07/R17/turn167academia16` | `CE-IN/PRIMARY-METHOD`；只保留 data-integration truth discovery 的 source false-positive/false-negative quality 模型及其数据集 | `NO_DIRECT_LOAD_BEARING_USE`；Claim/DD 为空 | source error-rate 模型不认证网络材料的作者、经历、独立性或未被操纵；joint 也只允许算法/数据集范围，不能桥到 `SS-04` |

仍保留直接身份但缩窄关系的还有：`S1/K07/R01` 与 `R03` 只保留 `EF-16`，`S1/K09/R16` 只保留 `EF-10`，`S1/K12/R19` 只保留 `SS-05`，`S2/K06/R05` 只保留 `TF-04` 中 quality-WTP/price 比较的形式模型窄桥。被删除的 `SS-11`、`EF-09`、`SS-06`、`SS-04` 与三条 `SS-01` 关系由 verifier 的冻结语义拒绝集和负向测试机械执行；任何未来重新提升都必须提出新的逐字 Claim、来源语义桥和独立审查，不能只改表格或计数。
...
- S1 的全部最终 CE-IN identity 必须在 JSONL 中各出现一次；S2 的全部条件式最终 CE-IN identity也必须在 JSONL 中各出现一次。
- 只有本文件人工表中的 identity 可在 JSONL 标记为 `DIRECT_LOAD_BEARING_MAPPING`；其余最终 CE-IN 必须为 `NO_DIRECT_LOAD_BEARING_USE`，且 `claim_ids/dd_ids` 为空。
- JSONL 的双方 claim、scope、CE 与 K 必须逐字/逐规则由 sealed ledgers 和 joint 重建；摘要或当前设计不能反向改写封存输入。
...
- 本 crosswalk/JSONL 与任一 joint、ledger 或 raw manifest 不一致时，以绑定的 sealed 原件为准，并使本文件进入 `INVALID`；不得静默修订原件。
