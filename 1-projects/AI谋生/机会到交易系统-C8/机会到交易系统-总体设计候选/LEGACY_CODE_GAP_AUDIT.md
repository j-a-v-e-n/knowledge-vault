...
本文件中的 `src/`、`tests/`、`pilot/`、`README.md`、`DESIGN.md` 与 `STATE.md` 都指向外部旧根 [`../机会到交易系统/`](../机会到交易系统/)，不是当前不可变候选内的文件。行号与测试基线绑定 C3 冻结前的审计快照；C3 终审期间旧 runtime/test 已发生并发变化，因此这些定位只证明历史差距，不能声称描述旧根当前字节。该漂移不会修复“不可作为新设计入口”的结论，也不能让本候选吸收旧代码。
